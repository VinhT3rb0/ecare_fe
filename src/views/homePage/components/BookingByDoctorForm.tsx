...
import { useCreateAppointmentMutation, useGetAvailableTimeSlotsQuery, useCheckAppointmentAvailabilityQuery } from "@/api/app_apointment/apiAppointment";
...
const BookingByDoctorForm: React.FC<BookingByDoctorFormProps> = ({ onClose }) => {
  ...
  const timeOptions = useMemo(() => {
    ...
    return slots.map((t: string) => {
      const isDisabled = isToday && dayjs(t, TIME_FMT).isBefore(now, 'minute');
      return {
        label: t,
        value: t,
        disabled: isDisabled
  ...
  const { data: availabilityCheck } = useCheckAppointmentAvailabilityQuery(
    ...
    {
      skip: !currentUserId || !doctorId || !date || !selectedTime
    }
  );
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      const errorMessage = err?.data?.message || err?.message || "Đặt lịch thất bại. Vui lòng thử lại!";
