...
import { CloseCircleOutlined, EyeOutlined, UserOutlined, CalendarOutlined } from "@ant-design/icons";

const { Text } = Typography;

export default function CancelledAppointments() {
    const { data, isLoading } = useGetAppointmentsByStatusQuery({ status: "cancelled" });
    const appointments = data?.data ?? [];
    const [selected, setSelected] = useState<any | null>(null);
    const [open, setOpen] = useState(false);

    if (isLoading) return <Spin />;
    if (!appointments.length) return <Empty description="Không có lịch hẹn đã hủy" />;
                ...
                {appointments.map((item: any) => (
                    <Col xs={24} sm={12} lg={8} key={item.id}>
                        <Card
                            ...
                            extra={<Tag color="red" icon={<CloseCircleOutlined />}>Đã hủy</Tag>}
                        >
