...
import { CheckSquareOutlined, EyeOutlined, UserOutlined, CalendarOutlined } from "@ant-design/icons";

const { Text } = Typography;

export default function CompletedAppointments() {
    const { data, isLoading } = useGetAppointmentsByStatusQuery({ status: "completed" });
    const appointments = data?.data ?? [];
    const [selected, setSelected] = useState<any | null>(null);
    const [open, setOpen] = useState(false);

    if (isLoading) return <Spin />;
    if (!appointments.length) return <Empty description="Không có lịch hẹn đã hoàn thành" />;
                ...
                {appointments.map((item: any) => (
                    <Col xs={24} sm={12} lg={8} key={item.id}>
                        <Card
                            ...
                            extra={<Tag color="green" icon={<CheckSquareOutlined />}>Đã hoàn thành</Tag>}
                        >
