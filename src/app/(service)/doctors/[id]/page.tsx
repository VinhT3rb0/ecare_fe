...
import { Spin, Card, Tag, Divider, Button, Space, Timeline } from "antd";
import {
    PhoneOutlined,
    CalendarOutlined,
    ArrowLeftOutlined,
                ...
                <aside className="lg:col-span-1">
                    <div className="sticky top-28">
                        <Card className="rounded-xl shadow-lg overflow-hidden">
                            ...
                            <div className="text-center mt-4">
                                ...
                                {doctor.education_level && (
                                    <p className="text-sm text-[#11A998] font-medium mt-1">
                                        {doctor.education_level}
                                    </p>
                                )}
                        ...
                        <Card className="mt-4 rounded-xl shadow-md">
                            <h3 className="font-semibold text-[#064E3B] mb-3">
                                Thông tin nhanh
                            </h3>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <Card className="rounded-xl shadow-md">
                        ...
                        <p className="text-sm text-gray-600">
                            {doctor.education_level} •{" "}
                            {doctor.Degrees?.[0]?.specialization || "Chuyên môn chưa cập nhật"}
                        </p>
                        <div className="mt-3 flex flex-wrap gap-2">
                            {doctor.departments?.map((d: any) => (
                                <Tag
                                    key={d.id}
                                    color="#11A998"
                                    className="text-[#064E3B] font-medium"
                                >
                                    {d.name}
                                </Tag>
                            ))}
                        ...
                        {doctor.Degrees?.length > 0 ? (
                            <Timeline mode="left">
                                {doctor.Degrees.map((deg: any) => (
                                    <Timeline.Item key={deg.id} label={deg.issue_date}>
                                        <h4 className="text-lg font-semibold text-[#064E3B]">
                                            {deg.specialization}
                                        </h4>
                                        <p className="text-sm text-gray-700">{deg.practice_scope}</p>
                                        <p className="text-xs text-gray-500 mt-1">
                                            Cấp tại {deg.issue_place}
                                        </p>
                                    </Timeline.Item>
                                ))}
                            </Timeline>
